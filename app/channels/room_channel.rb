...
class RoomChannel < ApplicationCable::Channel
  ...
  def speak(data)
    ...
    ActionCable.server.broadcast(
      "room_channel_#{message.room_id}", { id: message.id, user_id: message.user_id, message: message.message, user_name: message.user.nickname, created_at: message.created_at.strftime('%Y/%m/%d %H:%M') }
    )
  end
